...
Hinweis: Das Human-Gate schützt `/twitch/impressum` und `/twitch/agb`. `/twitch/datenschutz` ist öffentlich und indexierbar, damit Nutzer und OAuth-Prüfungen die Datenschutzerklärung direkt lesen können. Details zu Secrets, Caddy-Allowlist, CSP und Troubleshooting stehen in [`docs/LEGAL_ACCESS_GATE.md`](LEGAL_ACCESS_GATE.md).
