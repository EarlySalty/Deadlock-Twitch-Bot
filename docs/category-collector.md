...
Explizite Twitch-Moderationsereignisse sind ein getrennter Vorgang: CLEARMSG entfernt nur die konkret bezeichnete Nachricht einschließlich zugehöriger Shared-Chat-Kopien. Gespeicherte Ziel-IDs verhindern deren Wiederherstellung durch verspätete Wiederholungen. Ein personenbezogenes CLEARCHAT ist auf den aktuell beobachteten Stream begrenzt. Ein allgemeines CLEARCHAT ohne Ziel löscht kein Kanalarchiv. Stundenaggregate werden danach neu berechnet. Eine rechtlich oder vom Betreiber angeordnete Datenlöschung ist nicht Teil einer pauschalen Retention.
...
## Antwortschutz des Hauptbots

Der Collector löst grundsätzlich keine Bot-Aktionen aus. Zusätzlich lässt die Hauptbot-Pipeline bekannte Automationsbots, aktive ID-basierte Antwortsperren, aktuelle Scam-Belege und erkannte Spamtexte keine Begrüßungen, Befehlsantworten, Promos oder Unterhaltung auslösen. Das gilt unabhängig davon, ob ein Streamer automatische Moderation aktiviert hat. Globale Sperren werden auch bei ausgeschalteter Moderation für Antworten berücksichtigt.

Sperren liegen reversibel in `twitch_chat_response_blocks`. Es gibt keine Bot-Einstufung allein wegen stillen Zuschauens oder geringer Aktivität. Aufgehobene Sperren und korrigierte Scam-Urteile werden wieder freigegeben. Bei nicht prüfbarer Antwortsperre bleiben Unterhaltungsantworten gesperrt; die Archivierung ist davon getrennt. Die bestehende Kanal-Policy verweigert auch Raid-Schreibaktionen außerhalb freigegebener Partnerkanäle. Der separat autorisierte Outreach-Account bleibt ein eigener Anwendungsfall.
...
Die Unit hat 512 MiB Speicherlimit, CPU-Begrenzung, Restart-Backoff und eine getrennte OnFailure-Benachrichtigung. Die neue Migration und Rollenmatrix müssen vor dem neuen Bot-Start angewandt sein, damit der Antwortschutz seine Tabellen lesen kann.
