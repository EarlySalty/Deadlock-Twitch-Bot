## #98 — Keine doppelte Werbung mehr + Scam-Warnung erscheint wieder

**Problem:** Zwei Sachen an der Chat-Werbung waren kaputt. Erstens postete der Bot seine Discord-Werbung manchmal doppelt — zwei Werbungen mit nur einer einzigen Chat-Nachricht dazwischen. Eigentlich gibt es eine Mindestschwelle, wie viele Nachrichten zwischen zwei Werbungen liegen müssen, aber die wurde übergangen. Grund: Über das Werben entscheiden zwei Stellen parallel — eine reagiert auf jede eingehende Chat-Nachricht, eine läuft als Timer im Hintergrund. Beide prüfen die Schwelle, senden dann aber erst nach mehreren Zwischenschritten und setzen den „zuletzt geworben"-Marker erst ganz am Ende. Trifft eine Chat-Nachricht genau in dieses Zeitfenster, sehen beide noch den alten Stand und werben gleichzeitig.

Zweitens wurde die Warnung vor den Fake-/Scam-Discordservern nie ausgespielt. Sie hängt an einem eigenen Timer, der nur im Arbeitsspeicher lag und nirgends gespeichert wurde. Da der Bot nach jeder Änderung neu startet, wurde dieser Timer jedes Mal frisch auf Anfang „gesät" — die Warnung wäre erst bei einer zweiten Werbe-Gelegenheit fällig geworden, und die wird in einem einzelnen Bot-Lauf praktisch nie erreicht (Werbung kommt frühestens alle 90 Minuten). Der normale Werbe-Cooldown wird dagegen sehr wohl gespeichert — genau diese Schieflage ließ die Warnung dauerhaft hängen.

**Geändert:** Beide Werbe-Pfade werden jetzt pro Kanal gegeneinander gesperrt, sodass immer nur einer gleichzeitig entscheiden und senden darf. Der Scam-Warn-Timer wird genauso dauerhaft gespeichert wie der Werbe-Cooldown und übersteht Neustarts. Zusätzlich wurde der Warn-Abstand auf 2 Stunden gesetzt — länger als der Werbe-Takt, damit die Warnung regelmäßig, aber seltener als normale Werbung kommt.

**Wie's funktioniert:** Ein Schloss pro Kanal umschließt jetzt die komplette Sequenz „Schwelle prüfen → senden → Marker setzen". Während ein Pfad sendet, muss der andere warten; sobald er drankommt, sieht er den frisch gesetzten Marker und die noch nicht zurückgesetzte Nachrichtenschwelle und bricht ab. Doppelte Werbung ist damit ausgeschlossen, und die Mindestnachrichten-Schwelle greift wieder zuverlässig.

Der Warn-Timer landet jetzt in der Datenbank (wie die übrigen Werbe-Cooldowns) und wird beim Start zurückgeladen, statt bei jedem Neustart verloren zu gehen. Beim allerersten Mal pro Kanal wird er bewusst um „Cooldown minus kurze Anlaufzeit" in die Vergangenheit datiert: So feuert die Warnung nicht sofort beim Start (das wäre aus dem Kontext gerissen), aber schon bei der zweiten echten Werbe-Gelegenheit statt erst Stunden später. Tritt die Warnung auf, ersetzt sie wie bisher eine reguläre Werbung im selben Slot; danach läuft der 2-Stunden-Abstand, wodurch sich Werbung und Warnung von selbst abwechseln.
