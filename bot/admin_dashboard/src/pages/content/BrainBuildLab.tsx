...
const field = 'w-full rounded-xl border border-white/15 bg-[#131525] px-3 py-2 text-white';
...
function Purchase({ item, abilities }: { item: BrainItem; abilities: Map<number, string> }) {
  return <article className="rounded-xl border border-white/10 bg-white/[0.025] p-4">
    ...
    {item.sell_priority != null && <p className="mt-1 text-sm text-amber-200">Verkaufskandidat · Priorität {item.sell_priority}</p>}
...
function Scores({ scores }: { scores: BrainScore[] }) {
  ...
  return <section className={panel}>
    ...
    {!scores.length && <p className="mt-3 text-amber-200">Keine Scores dieser Variante vorhanden. Es werden keine Werte einer anderen Familie eingesetzt.</p>}
...
export default function BrainBuildLab() {
  ...
  return <div className="space-y-6">
    ...
    <div className="rounded-xl border border-amber-400/30 bg-amber-400/10 p-4 text-sm text-amber-100">Experimenteller Stand. Ein neuer Patch-Tag allein beweist keine aktuellen Itemdaten oder Nach-Patch-Matches. Quellenalter und offene Mechaniken bleiben sichtbar; ein erzeugter Build ist nicht automatisch fachlich freigegeben.</div>
    {error && <div role="alert" className="whitespace-pre-wrap rounded-xl border border-red-400/40 bg-red-400/10 p-4 text-red-100">{error}</div>}
    ...
    {catalog && <section className={panel}><p className="break-all text-sm text-text-secondary">Datenbank-Patch: <strong className="text-white">{catalog.patch_tag}</strong></p><div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-xs text-text-secondary">{catalog.sources.map(s => <span key={`${s.source}-${s.entity_type}`}>{s.entity_type}: {date(s.fetched_at)}</span>)}</div>
      ...
      </fieldset><button disabled={busy || !input.hero} type="submit" className="mt-5 flex items-center gap-2 rounded-xl bg-primary px-5 py-3 font-semibold text-white disabled:opacity-50">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Brain className="h-4 w-4" />}{busy ? 'Rust-Reasoner berechnet …' : 'Builds neu berechnen'}</button></form>
    </section>}
    {report && plan && <>
      <section className={panel}><div className="flex flex-wrap items-center justify-between gap-4"><div><h2 className="text-xl font-semibold text-white">{report.hero.name}: {plans.length} berechnete Variante{plans.length === 1 ? '' : 'n'}</h2><p className="mt-1 text-sm text-text-secondary">Patch: {report.config.patch_tag} · {report.config.combat_window_seconds} s · Kanal {Math.round(report.config.channel_uptime * 100)} % · Sicherheit {plan.confidence}</p></div><button className="flex items-center gap-2 rounded-xl border border-white/15 px-4 py-2" onClick={exportReport}><Download className="h-4 w-4" />Prüfbericht als JSON</button></div>
        ...
        <details className="mt-4 text-sm" open><summary className="cursor-pointer font-semibold text-amber-200">Prüfhinweise und Grenzen ({report.warnings.length})</summary>{report.warnings.map((warning, i) => <p key={i} className="mt-2 text-amber-100/90">{warning}</p>)}</details>
      </section>
      ...
      <section className={panel}><h2 className="text-lg font-semibold text-white">Skillorder dieser Variante</h2>{plan.ability_order.length ? <div className="mt-3 flex flex-wrap gap-2">{plan.ability_order.map((step, i) => <span className="rounded-lg border border-white/10 px-3 py-2 text-sm" key={i}>{i + 1}. {abilities.get(step.ability_id) ?? `Fähigkeit ${step.ability_id}`} (+{step.delta})</span>)}</div> : <p className="mt-2 text-amber-200">Keine belastbare Skillorder vorhanden. Es wird keine Folge erfunden.</p>}</section>
