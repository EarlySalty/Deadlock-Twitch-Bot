...
const button =
  'min-h-11 rounded-lg border border-white/20 px-3 py-2 text-sm hover:border-warning focus-visible:outline-2 focus-visible:outline-warning disabled:cursor-not-allowed disabled:opacity-40';
const input =
  'min-h-11 w-full rounded-lg border border-white/15 bg-black/50 px-3 text-white focus-visible:outline-2 focus-visible:outline-warning';
  ...
  return (
    <div className="space-y-6 pb-8">
      {blocker.state === 'blocked' && (
        <div role="alert" className="rounded-xl border border-warning/40 bg-background p-4 text-white">
      ...
      {error && (
        <div role="alert" className="rounded-xl border border-danger/50 bg-danger/40 p-4 text-danger">
          {error}
          <button
        ...
        <>
          <section className="panel-card space-y-4 rounded-2xl p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <span className={`rounded-full px-3 py-1 text-sm ${dirty ? 'bg-warning/15 text-warning' : 'bg-success/15 text-success'}`}>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {(Object.keys(layoutMeta) as CasterLayout[]).map((layout) => {
                ...
                return (
                  <button
                    key={layout}
                    className={`rounded-xl border p-4 text-left transition ${active ? 'border-warning bg-warning/10' : 'border-white/15 bg-black/30 hover:border-white/30'}`}
            ...
            <div className={`grid gap-3 ${slotCount === 1 ? 'grid-cols-1' : slotCount === 2 ? 'md:grid-cols-2' : 'md:grid-cols-3'}`}>
              {Array.from({ length: slotCount }, (_, index) => (
                <div
                  ...
                  className={`rounded-xl border p-3 ${dragged ? 'border-warning/60 bg-warning/5' : 'border-white/10 bg-black/25'}`}
                >
            ...
            <div className="flex flex-wrap gap-2">
              {scene.roster.map((person) => (
                <span
                  ...
                  className="cursor-grab rounded-lg border border-warning/30 bg-black/40 px-3 py-2 text-sm text-warning"
                >
                  ⠿ {person.name}
                </span>
              ))}
          ...
          <section className="panel-card space-y-4 rounded-2xl p-5">
            ...
            {createdCamera && (
              <div className="rounded-xl border border-warning/35 bg-warning/5 p-4">
                <strong className="text-warning">Link jetzt an @{createdCamera.ownerLogin} schicken</strong>
              ...
              {cameras.map((camera) => (
                <div key={camera.cameraId} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/10 bg-black/25 p-4">
                  <div>
                    ...
                    <div className={`mt-2 text-sm ${camera.online ? 'text-success' : camera.consentEnabled ? 'text-warning' : 'text-white/45'}`}>
          ...
          <div className="sticky bottom-3 z-20 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-warning/40 bg-background p-4 shadow-xl">
            <p role="status" className="text-sm text-white/75">{status || 'Bereit.'}</p>
            <button
              className={`${button} border-warning bg-warning/15 font-semibold text-warning`}
