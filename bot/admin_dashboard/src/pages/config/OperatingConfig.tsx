...
const fields: Array<{ key: keyof OperatingOptions; label: string; hint: string; min: number; max: number }> = [
  ...
  { key: 'acquire_timeout_ms', label: 'Warten auf eine freie Verbindung (Millisekunden)', hint: 'Danach meldet der Dienst einen Fehler, statt weiter auf den Pool zu warten.', min: 100, max: 300000 },
  { key: 'connect_timeout_seconds', label: 'Verbindungsaufbau (Sekunden)', hint: 'Zeitlimit für den Aufbau einer neuen Datenbankverbindung.', min: 1, max: 300 },
];
          ...
          {fields.map(field => <div key={field.key}>
            ...
            <input id={`operating-${field.key}`} type="number" min={field.min} max={field.max} step={1} required
              ...
              onChange={event => {
                ...
                if (!base && query.data) setBase(query.data.saved_fingerprint);
                setNotice('');
                save.reset();
              }} />
