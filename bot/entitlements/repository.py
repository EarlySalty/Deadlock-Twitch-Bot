"""Shared entitlement persistence helpers."""
...
ACTIVE_BILLING_STATUSES = ("active", "trialing", "past_due")


def is_missing_current_period_end_error(exc: Exception) -> bool:
...
def is_missing_manual_override_metadata_error(exc: Exception) -> bool:
    message = str(exc).strip().lower()
...
def normalize_candidate_refs(refs: list[str] | tuple[str, ...]) -> list[str]:
    normalized: list[str] = []
    seen: set[str] = set()
    for ref in refs:
...
def manual_override_from_row(row: Any) -> dict[str, Any] | None:
    plan_id = str(row_value(row, "manual_plan_id", 2, "") or "").strip()
    if plan_id not in KNOWN_PLAN_IDS:
        return None
    expires_at = parse_datetime_value(row_value(row, "manual_plan_expires_at", 3, None))
    is_active = not bool(expires_at and expires_at < datetime.now(UTC))
    return {
        "twitch_user_id": str(row_value(row, "twitch_user_id", 0, "") or "").strip(),
        "twitch_login": str(row_value(row, "twitch_login", 1, "") or "").strip(),
        "plan_id": plan_id,
        "expires_at": expires_at.isoformat() if expires_at else None,
        "notes": str(row_value(row, "manual_plan_notes", 4, "") or "").strip(),
        "updated_at": str(row_value(row, "manual_plan_updated_at", 5, "") or "").strip() or None,
        "is_active": is_active,
        "is_expired": not is_active,
    }


def load_manual_override(conn: Any, refs: list[str]) -> dict[str, Any] | None:
    for ref in refs:
        try:
            row = conn.execute(
                """
            ...
            if not is_missing_manual_override_metadata_error(exc):
                raise
            row = conn.execute(
                """
                SELECT
        ...
        payload = manual_override_from_row(row)
        if payload:
            return payload
    return None


def load_billing_subscription(conn: Any, refs: list[str]) -> dict[str, Any] | None:
    for ref in refs:
        try:
            row = conn.execute(
                """
                ...
                LIMIT 1
                """,
                (ref, *ACTIVE_BILLING_STATUSES),
            ).fetchone()
            current_period_end = row_value(row, "current_period_end", 3, None)
            updated_at = row_value(row, "updated_at", 4, "")
        except Exception as exc:
            if not is_missing_current_period_end_error(exc):
                raise
            row = conn.execute(
                """
            ...
            updated_at = row_value(row, "updated_at", 3, "")
        if not row:
            continue
        status = str(row_value(row, "status", 2, "") or "").strip().lower()
        if status not in ACTIVE_BILLING_STATUSES:
            continue
        return {
            "customer_reference": str(row_value(row, "customer_reference", 0, ref) or ref).strip(),
            "plan_id": normalize_plan_id(row_value(row, "plan_id", 1, "raid_free")),
            "status": status,
            "current_period_end": parse_datetime_value(current_period_end),
...
def resolve_plan_snapshot(
    conn: Any,
    refs: list[str] | tuple[str, ...],
    *,
    fallback_ref: str = "",
) -> dict[str, Any]:
    normalized_refs = normalize_candidate_refs(refs)
    ...
    manual_override = load_manual_override(conn, normalized_refs)
    billing_subscription = load_billing_subscription(conn, normalized_refs)
    return build_plan_snapshot(
        manual_override=manual_override,
        billing_subscription=billing_subscription,
