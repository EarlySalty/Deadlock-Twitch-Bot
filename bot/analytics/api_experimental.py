...
class _AnalyticsExperimentalMixin:
    ...
    def _load_exp_overview_payload(self, streamer: str, since: str) -> dict[str, object]:
    ...
    def _load_exp_game_breakdown_payload(self, streamer: str, since: str) -> list[dict[str, object]]:
        with storage.readonly_connection() as conn:
            rows = conn.execute(
                """
                SELECT
    ...
    def _load_exp_game_transitions_payload(
        self, streamer: str, since: str
    ...
    def _load_exp_growth_curves_payload(
        self, streamer: str, since: str
    ) -> list[dict[str, object]]:
        with storage.readonly_connection() as conn:
            rows = conn.execute(
    ...
    async def _api_v2_exp_game_breakdown(self, request: web.Request) -> web.Response:
        ...
        try:
            payload = await asyncio.to_thread(
                self._load_exp_game_breakdown_payload,
                streamer,
                since,
            )
            return web.json_response(payload)
    ...
    async def _api_v2_exp_game_transitions(self, request: web.Request) -> web.Response:
        ...
        try:
            payload = await asyncio.to_thread(
                self._load_exp_game_transitions_payload,
                streamer,
                since,
            )
            return web.json_response(payload)
    ...
    async def _api_v2_exp_growth_curves(self, request: web.Request) -> web.Response:
        ...
        try:
            payload = await asyncio.to_thread(
                self._load_exp_growth_curves_payload,
                streamer,
                since,
            )
            return web.json_response(payload)
