        ...
        >
          ...
          >
            ...
            {editorMode === 'advanced' && layout === 'canvas' && snapHits.length > 0 && <svg data-testid="overlay-snap-guides" aria-hidden="true" className="pointer-events-none absolute inset-0 z-30" width="100%" height="100%">
              {snapHits.map(hit => <line key={hit.axis} data-axis={hit.axis} data-position={hit.position} x1={hit.axis === 'x' ? `${hit.position / canvasWidth * 100}%` : 0} x2={hit.axis === 'x' ? `${hit.position / canvasWidth * 100}%` : '100%'} y1={hit.axis === 'y' ? `${hit.position / canvasHeight * 100}%` : 0} y2={hit.axis === 'y' ? `${hit.position / canvasHeight * 100}%` : '100%'} stroke="var(--color-success)" strokeWidth={1.5} />)}
            </svg>}
          </div>
          </div>
