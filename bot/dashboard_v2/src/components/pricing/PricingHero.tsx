...
async function startTrial(): Promise<TrialState> {
  try {
    ...
    // liefert csrfToken:null → harter Header-Zwang erzeugte Prod-403, Vorfall #1A1210).
  ...
  return (
    <section className="relative text-center mb-12 overflow-hidden">
      <div className="absolute inset-0 -z-10">
        <div className="absolute inset-0 bg-gradient-to-b from-[#C5A059]/10 via-transparent to-transparent" />
        <div
          className="absolute inset-0 opacity-30"
          style={{ background: 'radial-gradient(ellipse 80% 50% at 50% 0%, rgba(197, 160, 89, 0.15), transparent 70%)' }}
        />
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#C5A059]/10 border border-[#C5A059]/20 mb-6">
          ...
          <span className="text-sm font-medium text-[#C5A059]">Dein Growth Coach für Twitch</span>
        </div>

        <h1 className="text-4xl md:text-5xl font-bold text-white mb-4 tracking-tight">
          Mehr Wachstum, mehr{' '}
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#C5A059] to-[#00D9FF]">
            Insights
          </span>
        </h1>
        ...
        <button
          ...
          className={`inline-flex items-center gap-2 px-8 py-4 rounded-xl font-semibold text-lg transition-[transform,translate,scale,background-color,box-shadow,color] duration-200 ${
            trialState === 'granted'
        ...
        <div className="mt-8 pt-6 border-t border-white/8 flex flex-col sm:flex-row items-center justify-between gap-4 max-w-xl mx-auto">
          <div className="flex items-center gap-3 text-left">
            <div className="flex-shrink-0 w-9 h-9 rounded-xl bg-gradient-to-br from-[#C5A059]/20 to-[#00D9FF]/20 flex items-center justify-center border border-[#C5A059]/25">
          ...
          <a
            href="#plans"
            className="flex-shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-xl bg-white/6 hover:bg-white/10 border border-white/12 text-white/70 hover:text-white text-sm font-medium transition-[transform,translate,scale,background-color,box-shadow,color] duration-200"
          >
            Mehr erfahren
