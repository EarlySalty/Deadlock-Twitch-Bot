...
const PRIORITY_STYLES: Record<string, { bg: string; border: string; text: string; badge: string }> = {
  ...
  medium: { bg: 'bg-primary/10', border: 'border-primary/30', text: 'text-primary', badge: 'bg-primary text-[#0D0806]' },
        ...
        <BarChart data={chartData}>
          ...
          <Bar dataKey="avgViewers" name="avgViewers" radius={[4, 4, 0, 0]}>
            {chartData.map((entry, idx) => (
              <Cell
                key={idx}
                fill={entry.isOptimal ? '#00FF88' : 'rgba(0, 217, 255, 0.6)'}
                stroke={entry.isOptimal ? '#00FF88' : 'transparent'}
              ...
              {hours.map(hour => {
                ...
                return (
                  <div
                    ...
                    style={{
                      backgroundColor: `rgba(0, 255, 136, ${intensity * 0.8 + 0.05})`,
                      minWidth: '12px',
                      minHeight: '12px',
                    }}
      ...
      {chartData.length > 1 && (
        <ResponsiveContainer width="100%" height={250}>
          <LineChart data={chartData}>
            ...
            <Line type="monotone" dataKey="top" stroke="#00D9FF" strokeWidth={2} dot={false} strokeDasharray="5 5" />
          </LineChart>
        </ResponsiveContainer>
      )}
    </motion.div>
...
export function CompetitionDensitySection({ data }: { data: CoachingData }) {
  ...
  return (
    ...
    >
              ...
              return (
                <div key={w.weekday} className="text-center">
                  ...
                  <div
                    ...
                    style={{
                      backgroundColor: `rgba(0, 217, 255, ${intensity * 0.4 + 0.05})`,
                      borderColor: w.yourData ? 'rgba(0, 217, 255, 0.6)' : 'transparent',
                    }}
                  >
      ...
      <div className="flex items-center gap-4 mt-4 text-xs text-text-secondary">
        <div className="flex items-center gap-1">
          <div className="w-3 h-3 rounded-sm" style={{ backgroundColor: 'rgba(0, 217, 255, 0.8)' }} />
