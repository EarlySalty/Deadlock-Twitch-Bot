...
| `tb-monitoring` | **4a–4e umgesetzt:** Guard-Store + Processing-Inbox (4a); Write-Core: Live-State/Sessions/Stats/exp (4b); Poll-Engine mit `StreamSource`/`AnnouncementSink`/`PollHooks`-Ports (4c); EventSub-Ingress: Dispatcher (Bridge-Vertrag, Message-Dedup 600 s), Inbox-Handler (online/offline/update mit Business-Effect-Guards), Telemetrie-Writes, Subscription-Manager (Core-Subs App-Token, Cleanup, Capacity-Snapshot) + `EventSubHooks`-Port (4d); Announcement-Template-Engine (per-Streamer-Configs, Platzhalter, Cache-Buster) + Broker-Sink mit Retry-Token (4e). Folgt: Cutover-Wiring (4f). Webhook-only, kein WS-Pool — eigene `tb-eventsub`-Crate entfällt (ADR 0004). **Poll-Besonderheiten:** tracked Logins werden OHNE Sprachfilter abgefragt (kuratierte Partnerliste; Sprachfilter nur fürs Kategorie-Sampling) und stream.online macht ein Go-Live-Enrichment via `ChannelInfoSource`-Port (Helix `/channels`, setzt last_title/last_game sofort statt poll-abhängig) | db (sqlx direkt) — Helix/Broker-Adapter im Composition-Root `tb-bot` (`wiring.rs`) | sqlx, tokio, chrono, uuid, serde |
