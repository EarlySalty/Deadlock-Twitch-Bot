...
-- Bewusst NICHT in der Drop-Liste, obwohl ebenfalls null Scans:
--
--   idx_stream_ai_reports_streamer        legt post_stream.rs:2007 wieder an
--   idx_stream_ai_reports_session_variant legt post_stream.rs:2005 wieder an
--   idx_mmu_ts                            legt ledger.rs:131 wieder an
--
-- Diese drei entstehen beim naechsten Lauf der Runtime-Schema-Guards erneut
-- (CREATE INDEX IF NOT EXISTS). Ein Drop waere nicht nur wirkungslos, sondern
-- schaedlich: der Wiederaufbau liefe als Foreground-Indexbuild im Live-Pfad.
-- Zusammen sind es 712 kB gegenueber 219 MB beim Hauptposten -- den Guards
-- hinterherzuraeumen lohnt den Eingriff in fremde Crates nicht.
--
--   idx_clip_fetch_history_streamer  hat ebenfalls null Scans, aber aus einem
-- behebbaren Grund: scout.rs filterte mit LOWER(streamer_login), was den Index
-- auf der rohen Spalte ausschliesst. Der Query-Fix derselben Aenderung macht
-- ihn nutzbar, deshalb bleibt er.
