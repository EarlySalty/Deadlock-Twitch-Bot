...
async fn refresh(pool:&PgPool,helix:&HelixClient,token_manager:&BotTokenManager)->Result<(),String> {
    let exists:bool=sqlx::query_scalar("SELECT to_regclass('public.category_channels') IS NOT NULL")
    ...
    let ids:Vec<String>=sqlx::query_scalar("SELECT user_id FROM category_channels WHERE last_seen>now()-interval '1 day'
        ...
        ORDER BY followers_checked_at NULLS FIRST,user_id LIMIT 25").fetch_all(pool).await.map_err(|e|e.to_string())?;
    if ids.is_empty(){return Ok(());}
    let token=token_manager.access_token().await.map_err(|e|e.to_string())?;
    for id in ids {
        ...
        let result=helix.get_followers_total(&id,Some(&token)).await.map_err(|e|e.to_string())?;
