...
use crate::{Error,config::{Config,protected_text}};

pub async fn run(pool:PgPool,helix:Arc<HelixClient>,config:Arc<Config>,game_id:String)->Result<(),Error> {
    let mut timer=tokio::time::interval(Duration::from_secs(10));
    timer.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Skip);
    loop {
        timer.tick().await;
        // Metadata failures must not erase snapshots or reconnect anonymous chat.
        if let Err(error)=profiles(&pool,&helix,&config).await { tracing::warn!(%error,"public category profile enrichment failed"); }
        if config.media_enabled {
            if let Err(error)=media(&pool,&helix,&game_id).await { tracing::warn!(%error,"category media metadata enrichment failed"); }
        }
    }
}
async fn profiles(pool:&PgPool,helix:&HelixClient,config:&Config)->Result<(),Error> {
    let ids:Vec<String>=sqlx::query_scalar("SELECT user_id FROM category_channels WHERE last_seen>now()-interval '1 day'
        AND (profile_checked_at IS NULL OR profile_checked_at<now()-interval '1 day')
        ORDER BY profile_checked_at NULLS FIRST,user_id LIMIT 20").fetch_all(pool).await?;
    if ids.is_empty(){ return Ok(()); }
    let profiles=helix.get_public_profiles(&ids).await?;
    for profile in profiles {
        let created=DateTime::parse_from_rfc3339(&profile.created_at).ok().map(|d|d.with_timezone(&Utc));
    ...
    if let Some(file)=&config.follower_token_file {
        let token=protected_text(file)?;
        for id in ids {
            let total=helix.get_followers_total(&id,Some(token.trim().trim_start_matches("oauth:"))).await?;
            sqlx::query("UPDATE category_channels SET followers_total=CASE WHEN $2::bigint IS NULL THEN followers_total ELSE $2 END,
                followers_checked_at=now(),followers_http_status=$3 WHERE user_id=$1")
                .bind(id).bind(total.total).bind(total.http_status.map(i32::from)).execute(pool).await?;
...
async fn media(pool:&PgPool,helix:&HelixClient,game_id:&str)->Result<(),Error> {
    sqlx::query("INSERT INTO category_media_jobs(user_id,kind) SELECT user_id,kind FROM category_channels
        ...
        .execute(pool).await?;
    let job=sqlx::query("SELECT user_id,kind,cursor,window_start,window_end,pages FROM category_media_jobs
        WHERE next_at<=now() ORDER BY next_at,user_id LIMIT 1").fetch_optional(pool).await?;
    ...
    let result=helix.public_media_page(kind=="clip",&user,cursor.as_deref(),&since.to_rfc3339(),&until.to_rfc3339()).await;
    ...
    for item in items {
        let Some(id)=item.get("id").and_then(|v|v.as_str()).filter(|s|!s.is_empty()) else { continue; };
        ...
        let verified=kind=="clip" && item.get("game_id").and_then(|v|v.as_str())==Some(game_id);
    ...
    let bounded=pages>=999 || (kind=="clip" && pages>=9 && next.is_some());
    if repeated || bounded {
