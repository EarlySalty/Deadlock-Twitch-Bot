...
use crate::retention::{delete_clips_by_ids, is_clip_published_on_all_active_platforms, iter_expired_clips_for_retention};
...
impl RetentionWorker {
    pub fn new(pool: PgPool) -> Self {
        Self { pool, interval: Duration::from_secs(INTERVAL_SECS) }
    }
    ...
    pub async fn run_once(&self) {
        ...
        for clip in candidates {
            ...
            if clip.discarded_at.is_none() && !is_clip_published_on_all_active_platforms(&self.pool, clip.id).await {
                continue;
            }
...
mod tests {
    ...
    async fn cleanup_loescht_nur_fertige_clips() {
        let Some(pool) = make_pool("t_sm_retention_worker").await else { return };
        ...
        let remaining: Vec<i32> = sqlx::query_scalar("SELECT id FROM twitch_clips_social_media ORDER BY id").fetch_all(&pool).await.unwrap();
