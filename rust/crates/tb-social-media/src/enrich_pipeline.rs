...
mod tests {
    ...
    impl Transcriber for MockTranscriber {
        async fn transcribe_clip(&self, _: &Path) -> Result<TranscriptionOutput, TranscribeError> {
            self.out
                .clone()
                .ok_or(TranscribeError::Unavailable("no model".into()))
        }
    }
    ...
    impl EnrichmentLlm for MockLlm {
        async fn generate(&self, _: &LlmRequest) -> Result<LlmResponse, LlmError> {
            self.response
                .clone()
                .ok_or(LlmError::ProviderError("all failed".into()))
        }
    }

    fn llm_response() -> LlmResponse {
        let pe = |t: &str| PlatformEnrichment {
            title: Some(t.into()),
    ...
    async fn voller_lauf_done() {
        let Some(pool) = make_pool("t_sm_pipe_done").await else {
            return;
        };
        ...
        let transcriber = MockTranscriber {
            out: Some(TranscriptionOutput {
                text: "haze ist stark".into(),
                segments: vec![serde_json::json!({"text":"haze"})],
                language: Some("de".into()),
            }),
        };
        let llm = MockLlm {
            response: Some(llm_response()),
        };
    ...
    async fn skipped_no_key_wenn_llm_fehlt_und_transkription_uebersprungen() {
        let Some(pool) = make_pool("t_sm_pipe_skip").await else {
            return;
        };
        let id = seed_clip(&pool).await;
        let pipe = ClipEnrichmentPipeline::new(pool.clone());
    ...
    async fn clip_nicht_gefunden() {
        let Some(pool) = make_pool("t_sm_pipe_404").await else {
            return;
        };
