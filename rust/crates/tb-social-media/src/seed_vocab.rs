...
async fn fetch_json(http: &reqwest::Client, url: &str) -> Vec<Value> {
    let Ok(endpoint) = crate::http_security::endpoint(url) else {
        tracing::warn!("Deadlock-API endpoint requires HTTPS or literal loopback");
        return Vec::new();
    };
...
pub async fn seed_vocab(pool: &PgPool, include_slang: bool, include_api: bool) -> (usize, usize) {
    let http = crate::http_security::client(std::time::Duration::from_secs(30));
    seed_vocab_with(
        pool,
        &http,
        include_slang,
