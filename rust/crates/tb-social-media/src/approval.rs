...
type Row = (i32, Option<String>, Option<String>, Option<String>, Option<String>, Option<String>, Option<String>, Option<String>);
...
pub async fn get_approval_record(pool: &PgPool, clip_db_id: i32) -> Option<ApprovalRecord> {
    let row: Option<Row> = sqlx::query_as(SELECT_SQL).bind(clip_db_id).fetch_optional(pool).await.ok().flatten();
...
pub async fn ensure_approval_row(pool: &PgPool, clip_db_id: i32) -> ApprovalRecord {
    ...
    let _ = sqlx::query(
        "INSERT INTO social_media_clip_approval (clip_db_id, state, approved_platforms) \
         VALUES ($1, $2, '[]') ON CONFLICT (clip_db_id) DO NOTHING",
    )
    .bind(clip_db_id)
    .bind(STATE_AWAITING)
    .execute(pool)
    .await;
    get_approval_record(pool, clip_db_id).await.unwrap_or(ApprovalRecord {
        clip_db_id,
...
pub async fn mark_clip_awaiting_approval(pool: &PgPool, clip_db_id: i32) {
    ...
    let _ = sqlx::query(
        "UPDATE social_media_clip_approval SET state = $1, approver_user_id = NULL, \
         decided_at = NULL, approved_platforms = '[]', dm_message_id = NULL, \
...
pub async fn is_clip_approved_for(pool: &PgPool, clip_db_id: i32, platform: &str) -> bool {
    let platform = platform.trim().to_lowercase();
    if !SUPPORTED_PLATFORMS.contains(&platform.as_str()) {
        return false;
    }
    match get_approval_record(pool, clip_db_id).await {
...
fn auto_platforms(settings: AutoApprove) -> Vec<String> {
    let mut out = Vec::new();
    if settings.youtube { out.push("youtube".to_string()); }
    if settings.tiktok { out.push("tiktok".to_string()); }
    if settings.instagram { out.push("instagram".to_string()); }
    out
}
...
pub async fn handle_decision(
    ...
    sqlx::query(
        "UPDATE social_media_clip_approval SET state = $1, approved_platforms = $2::jsonb, \
         approver_user_id = $3, decided_at = $4::timestamptz WHERE clip_db_id = $5",
    )
    .bind(next_state)
...
pub async fn ensure_queued_uploads(pool: &PgPool, clip_db_id: i32) -> Vec<(String, i32)> {
    if !clip_exists(pool, clip_db_id).await {
        return Vec::new();
    }
    ...
    for platform in &record.approved_platforms {
        if !is_clip_approved_for(pool, clip_db_id, platform).await {
            continue;
        }
        if upload_already_exists(pool, clip_db_id, platform).await {
            continue;
        }
        let (title, description, hashtags) = enrichment.as_ref().map_or((None, None, Vec::new()), |e| match platform.as_str() {
            "youtube" => (e.title_youtube.clone(), e.description_youtube.clone(), e.hashtags_youtube.clone()),
            "tiktok" => (e.title_tiktok.clone(), e.description_tiktok.clone(), e.hashtags_tiktok.clone()),
            _ => (e.title_instagram.clone(), e.description_instagram.clone(), e.hashtags_instagram.clone()),
        });
        if let Ok(queue_id) = queue_upload(pool, clip_db_id, platform, title.as_deref(), description.as_deref(), Some(&hashtags), None, 0).await {
...
async fn upload_already_exists(pool: &PgPool, clip_db_id: i32, platform: &str) -> bool {
    ...
    let row: Option<(Option<i32>, bool)> = sqlx::query_as(&format!(
    ...
    .flatten();
    match row {
        Some((uploaded, has_queue)) => uploaded.unwrap_or(0) != 0 || has_queue,
        None => true,
    }
}
...
mod tests {
    ...
    async fn make_pool(schema: &str) -> Option<PgPool> {
        ...
        for ddl in [
            "CREATE TABLE twitch_clips_social_media (id SERIAL PRIMARY KEY, status TEXT DEFAULT 'pending', uploaded_tiktok INTEGER DEFAULT 0, uploaded_youtube INTEGER DEFAULT 0, uploaded_instagram INTEGER DEFAULT 0)",
    ...
    async fn seed_clip(pool: &PgPool) -> i32 {
        sqlx::query_scalar("INSERT INTO twitch_clips_social_media DEFAULT VALUES RETURNING id").fetch_one(pool).await.unwrap()
    }

    #[tokio::test]
    async fn approve_setzt_state_und_queued_uploads() {
        let Some(pool) = make_pool("t_sm_approval").await else { return };
        ...
        let rec = handle_decision(&pool, clip, "approve", &["youtube".to_string()], Some("admin")).await.unwrap();
        ...
        let cstatus: String = sqlx::query_scalar("SELECT status FROM twitch_clips_social_media WHERE id = $1").bind(clip).fetch_one(&pool).await.unwrap();
        assert_eq!(cstatus, "approved");
        // Upload eingereiht mit Enrichment-Titel.
        let (platform, title): (String, Option<String>) = sqlx::query_as("SELECT platform, title FROM twitch_clips_upload_queue WHERE clip_id = $1").bind(clip).fetch_one(&pool).await.unwrap();
        assert_eq!(platform, "youtube");
        assert_eq!(title.as_deref(), Some("YT-Titel"));
        assert!(is_clip_approved_for(&pool, clip, "youtube").await);
        assert!(!is_clip_approved_for(&pool, clip, "tiktok").await);

        // ensure_queued_uploads erneut → idempotent (kein zweiter Job).
        ensure_queued_uploads(&pool, clip).await;
        let n: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM twitch_clips_upload_queue WHERE clip_id = $1").bind(clip).fetch_one(&pool).await.unwrap();
        assert_eq!(n, 1);

        // iter_approved findet den Clip.
        assert_eq!(iter_approved_clips_pending_queue(&pool, 10).await, vec![clip]);
    }

    #[tokio::test]
    async fn skip_und_approve_ohne_plattform() {
        let Some(pool) = make_pool("t_sm_approval_skip").await else { return };
        let clip = seed_clip(&pool).await;
        let rec = handle_decision(&pool, clip, "skip", &[], None).await.unwrap();
        ...
        assert!(matches!(handle_decision(&pool, clip, "approve", &[], None).await, Err(ApprovalError::NoPlatform)));
        // Unbekannter Clip → ClipNotFound.
        assert!(matches!(handle_decision(&pool, 999, "approve", &["youtube".to_string()], None).await, Err(ApprovalError::ClipNotFound(999))));
    }

    #[tokio::test]
    async fn mark_awaiting_und_record() {
        let Some(pool) = make_pool("t_sm_approval_mark").await else { return };
        let clip = seed_clip(&pool).await;
        handle_decision(&pool, clip, "approve", &["tiktok".to_string()], None).await.unwrap();
