...
pub async fn get_global_templates(pool: &PgPool, category: Option<&str>) -> Vec<GlobalTemplate> {
    ...
    let rows = match category {
        Some(cat) => sqlx::query(&format!("{base} WHERE category = $1 ORDER BY usage_count DESC, template_name ASC"))
            .bind(cat).fetch_all(pool).await,
        None => sqlx::query(&format!("{base} ORDER BY usage_count DESC, template_name ASC")).fetch_all(pool).await,
    }
    .unwrap_or_default();
    rows.iter()
        .map(|r| GlobalTemplate {
            ...
            hashtags: parse_hashtags(r.try_get::<Option<String>, _>("hashtags").unwrap_or(None).as_deref()),
...
pub async fn create_streamer_template(
    ...
    let default_int = i32::from(is_default);
    if is_default {
        sqlx::query("UPDATE clip_templates_streamer SET is_default = 0 WHERE streamer_login = $1")
            .bind(streamer_login)
            .execute(pool)
            .await?;
    }
    let existing: Option<(i32,)> = sqlx::query_as(
        "SELECT id FROM clip_templates_streamer WHERE streamer_login = $1 AND template_name = $2",
    )
    .bind(streamer_login)
    .bind(template_name)
    .fetch_optional(pool)
    ...
    if let Some((id,)) = existing {
        sqlx::query(
            "UPDATE clip_templates_streamer SET description_template = $1, hashtags = $2, \
             is_default = $3, updated_at = $4 WHERE id = $5",
        )
        .bind(description_template)
        .bind(dump_hashtags(hashtags))
        .bind(default_int)
...
pub async fn get_streamer_templates(pool: &PgPool, streamer_login: &str) -> Vec<StreamerTemplate> {
    let rows = sqlx::query(
        "SELECT id, streamer_login, template_name, description_template, hashtags, is_default, \
    ...
    rows.iter()
        .map(|r| StreamerTemplate {
            id: r.try_get("id").unwrap_or(0),
            streamer_login: r.try_get("streamer_login").unwrap_or_default(),
            template_name: r.try_get("template_name").unwrap_or_default(),
            description_template: r.try_get("description_template").unwrap_or_default(),
            hashtags: parse_hashtags(r.try_get::<Option<String>, _>("hashtags").unwrap_or(None).as_deref()),
            is_default: r.try_get::<Option<i32>, _>("is_default").unwrap_or(None).unwrap_or(0) != 0,
            created_at: r.try_get("created_at").unwrap_or(None),
...
pub async fn apply_template_to_clip(pool: &PgPool, clip_id: i32, template_id: i32, is_global: bool) -> bool {
    let clip: Option<(Option<String>, Option<String>, Option<String>)> = sqlx::query_as(
    ...
    .ok()
    .flatten();
    let Some((title, streamer, game)) = clip else {
        return false;
    };

    let template: Option<(String, Option<String>)> = if is_global {
        let t = sqlx::query_as("SELECT description_template, hashtags FROM clip_templates_global WHERE id = $1")
            .bind(template_id).fetch_optional(pool).await.ok().flatten();
        let _ = sqlx::query("UPDATE clip_templates_global SET usage_count = usage_count + 1 WHERE id = $1")
            .bind(template_id).execute(pool).await;
        t
    } else {
        sqlx::query_as("SELECT description_template, hashtags FROM clip_templates_streamer WHERE id = $1")
            .bind(template_id).fetch_optional(pool).await.ok().flatten()
    };
    ...
    let streamer = streamer.unwrap_or_default();
    let game = game.filter(|g| !g.is_empty()).unwrap_or_else(|| "Unknown".to_string());
    ...
    sqlx::query("UPDATE twitch_clips_social_media SET custom_description = $1, hashtags = $2 WHERE id = $3")
        .bind(&description)
        .bind(dump_hashtags(&hashtags))
        .bind(clip_id)
        .execute(pool)
...
pub async fn save_last_hashtags(pool: &PgPool, streamer_login: &str, hashtags: &[String]) {
    let now = chrono::Utc::now().to_rfc3339();
    let _ = sqlx::query(
        "INSERT INTO clip_last_hashtags (streamer_login, hashtags, last_used_at) VALUES ($1, $2, $3) \
...
pub async fn get_last_hashtags(pool: &PgPool, streamer_login: &str) -> Vec<String> {
    let raw: Option<String> = sqlx::query_scalar("SELECT hashtags FROM clip_last_hashtags WHERE streamer_login = $1")
        .bind(streamer_login)
        .fetch_optional(pool)
        .await
        .ok()
...
mod tests {
    ...
    async fn global_template_crud_und_apply() {
        let Some(pool) = make_pool("t_sm_tpl").await else { return };
        let tid = create_global_template(&pool, "gaming", "{{streamer}} spielt {{game}}: {{title}}", &tags(&["#deadlock", "#{{game}}"]), Some("Gaming"), "admin").await.unwrap();
        ...
        let (desc, ht): (Option<String>, Option<String>) = sqlx::query_as("SELECT custom_description, hashtags FROM twitch_clips_social_media WHERE id = $1").bind(clip).fetch_one(&pool).await.unwrap();
        assert_eq!(desc.as_deref(), Some("nani spielt Dead Lock: Insane 1v3"));
        // {{game}} im Hashtag → ohne Leerzeichen.
        assert_eq!(parse_hashtags(ht.as_deref()), tags(&["#deadlock", "#DeadLock"]));
        // usage_count erhöht.
        let uc: i32 = sqlx::query_scalar("SELECT usage_count FROM clip_templates_global WHERE id = $1").bind(tid).fetch_one(&pool).await.unwrap();
        assert_eq!(uc, 1);
    }

    #[tokio::test]
    async fn streamer_template_upsert_und_default() {
        let Some(pool) = make_pool("t_sm_tpl_streamer").await else { return };
        let id1 = create_streamer_template(&pool, "nani", "main", "A", &tags(&["#a"]), true).await.unwrap();
        // Zweites Default → erstes wird zurückgesetzt.
        let id2 = create_streamer_template(&pool, "nani", "alt", "B", &tags(&["#b"]), true).await.unwrap();
        ...
        let id1b = create_streamer_template(&pool, "nani", "main", "A2", &tags(&["#a2"]), false).await.unwrap();
        assert_eq!(id1, id1b);
        assert_eq!(get_streamer_templates(&pool, "nani").await.iter().find(|t| t.id == id1).unwrap().description_template, "A2");
    }

    #[tokio::test]
    async fn last_hashtags_roundtrip() {
        let Some(pool) = make_pool("t_sm_lasttags").await else { return };
        assert!(get_last_hashtags(&pool, "nani").await.is_empty());
        save_last_hashtags(&pool, "nani", &tags(&["#deadlock", "#haze"])).await;
        assert_eq!(get_last_hashtags(&pool, "nani").await, tags(&["#deadlock", "#haze"]));
