...
impl ApprovalWorker {
    pub fn new(pool: PgPool) -> Self {
        Self { pool, batch_size: BATCH_SIZE, interval: Duration::from_secs(INTERVAL_SECS) }
    }
...
mod tests {
    ...
    async fn queued_nur_approved_clips() {
        let Some(pool) = make_pool("t_sm_approval_worker").await else { return };
        // Clip A: approved für tiktok, mit Enrichment-Titel.
        let a: i32 = sqlx::query_scalar("INSERT INTO twitch_clips_social_media DEFAULT VALUES RETURNING id").fetch_one(&pool).await.unwrap();
        ...
        let b: i32 = sqlx::query_scalar("INSERT INTO twitch_clips_social_media DEFAULT VALUES RETURNING id").fetch_one(&pool).await.unwrap();
        ...
        let (platform, title): (String, Option<String>) = sqlx::query_as("SELECT platform, title FROM twitch_clips_upload_queue WHERE clip_id = $1").bind(a).fetch_one(&pool).await.unwrap();
        assert_eq!(platform, "tiktok");
        assert_eq!(title.as_deref(), Some("TT-Titel"));
        // B: keine Queue-Zeile.
        let n_b: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM twitch_clips_upload_queue WHERE clip_id = $1").bind(b).fetch_one(&pool).await.unwrap();
        assert_eq!(n_b, 0);

        // Idempotent: zweiter Lauf legt keine Duplikate an.
        ApprovalWorker::new(pool.clone()).run_once().await;
        let n_a: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM twitch_clips_upload_queue WHERE clip_id = $1").bind(a).fetch_one(&pool).await.unwrap();
        assert_eq!(n_a, 1);
    }
}
