...
impl TokenRefreshWorker {
    ...
    pub async fn run_once(&self) {
        let threshold = (Utc::now() + chrono::Duration::hours(REFRESH_THRESHOLD_HOURS)).to_rfc3339();
        let rows = sqlx::query(
            "SELECT platform, streamer_login, refresh_token_enc, client_id, client_secret_enc, \
        ...
        for row in rows {
            self.refresh_one(&row).await;
        }
    }

    async fn refresh_one(&self, row: &sqlx::postgres::PgRow) {
        let platform: String = row.try_get("platform").unwrap_or_default();
        let streamer: Option<String> = row.try_get("streamer_login").unwrap_or(None);
        let refresh_enc: Vec<u8> = match row.try_get("refresh_token_enc") {
            Ok(v) => v,
            Err(_) => return,
        };
        ...
        let Ok(refresh_token) = self
            .cipher
            .decrypt_field(&refresh_enc, &aad::social_media("refresh_token", &platform, streamer_ref, enc_version))
        else {
        ...
        let client_secret = client_secret_enc.and_then(|b| {
            self.cipher
                .decrypt_field(&b, &aad::social_media("client_secret", &platform, streamer_ref, enc_version))
                .ok()
        });

        let new_tokens = match self
            .oauth
            .refresh_token(&platform, &refresh_token, client_id.as_deref().unwrap_or(""), client_secret.as_deref().unwrap_or(""))
            .await
        {
            Ok(t) => t,
            Err(error) => {
        ...
        self.save_refreshed(&platform, streamer_ref, &new_tokens).await;
    }

    async fn save_refreshed(&self, platform: &str, streamer: Option<&str>, new_tokens: &crate::oauth::RefreshedTokens) {
        let Ok(access_enc) = self
            .cipher
            .encrypt_field(&new_tokens.access_token, &aad::social_media("access_token", platform, streamer, 1))
        else {
        ...
        let refresh_enc = new_tokens.refresh_token.as_ref().and_then(|t| {
            self.cipher.encrypt_field(t, &aad::social_media("refresh_token", platform, streamer, 1)).ok()
        });
...
mod tests {
    ...
    async fn refresh_youtube_aktualisiert_access_behaelt_refresh() {
        let Some(pool) = make_pool("t_sm_refresh").await else { return };
        let c = cipher();
        // Ablaufender YouTube-Token (in 10min < 1h Threshold) mit Refresh.
        let soon = (Utc::now() + chrono::Duration::minutes(10)).to_rfc3339();
        let access_enc = c.encrypt_field("old-access", &aad::social_media("access_token", "youtube", None, 1)).unwrap();
        let refresh_enc = c.encrypt_field("the-refresh", &aad::social_media("refresh_token", "youtube", None, 1)).unwrap();
        sqlx::query(
        ...
        let oauth = OAuthManager::new(pool.clone(), c.clone()).with_token_urls(
            "http://127.0.0.1:1".into(), format!("{}/token", server.uri()), "http://127.0.0.1:1".into(),
        );
        ...
        let dec = c.decrypt_field(&access_enc, &aad::social_media("access_token", "youtube", None, 1)).unwrap();
    ...
    async fn nicht_ablaufende_werden_uebersprungen() {
        let Some(pool) = make_pool("t_sm_refresh_skip").await else { return };
        let c = cipher();
        // Token läuft erst in 3h ab → außerhalb des 1h-Thresholds.
        let later = (Utc::now() + chrono::Duration::hours(3)).to_rfc3339();
        let access_enc = c.encrypt_field("a", &aad::social_media("access_token", "youtube", None, 1)).unwrap();
        let refresh_enc = c.encrypt_field("r", &aad::social_media("refresh_token", "youtube", None, 1)).unwrap();
        ...
        TokenRefreshWorker::new(pool.clone(), c.clone(), oauth).run_once().await;
        let access: Vec<u8> = sqlx::query_scalar("SELECT access_token_enc FROM social_media_platform_auth WHERE platform='youtube'").fetch_one(&pool).await.unwrap();
        assert_eq!(c.decrypt_field(&access, &aad::social_media("access_token", "youtube", None, 1)).unwrap(), "a");
    }
}
