...
async fn run_learn_capture(
    ...
    let capture = match capturer
        .capture(
            channel,
            learn_capture_seconds().max(0) as u64,
            &transcript_quality(),
            None,
        )
        .await
...
pub fn spawn_learn_jobs(learn: Arc<ReactionLearning>) {
    ...
    spawn_logged(
        "engagement_learn_capture",
        schedule_learn_capture(Arc::clone(&learn)),
    );
    spawn_logged(
        "engagement_learn_mapper",
        schedule_learn_mapper(Arc::clone(&learn)),
    );
