//! GET/POST `/twitch/api/v2/streamer/title-command-settings`.
//!
//! Streamer-Selbstbedienung fuer den KI-Titelbefehl `!title` auf
//! `streamer_plans.title_command_enabled` (INTEGER, Default 1).
...
#[derive(Deserialize)]
pub struct TitleCommandUpdate {
    pub title_command_enabled: bool,
}
...
const SELECT_SQL: &str = "SELECT COALESCE(title_command_enabled, 1) AS enabled \
       FROM streamer_plans \
...
pub async fn get_handler(
    ...
    Query(query): Query<TitleCommandQuery>,
) -> Response {
...
pub async fn post_handler(
    ...
    State(pool): State<PgPool>,
    Query(query): Query<TitleCommandQuery>,
    Json(body): Json<TitleCommandUpdate>,
) -> Response {
    let (login, user_id) = match resolve_target(&auth, &query.streamer) {
        Ok(t) => t,
        Err(resp) => return resp,
    };
    let flag = i32::from(body.title_command_enabled);
    ...
    let result = if !user_id.is_empty() {
        sqlx::query(
            "INSERT INTO streamer_plans (twitch_user_id, twitch_login, title_command_enabled) \
             ...
             DO UPDATE SET title_command_enabled = EXCLUDED.title_command_enabled, \
        ...
        sqlx::query(
            "UPDATE streamer_plans SET title_command_enabled = $2 \
    ...
    match result {
        Ok(res) if res.rows_affected() > 0 => {
            Json(json!({ "ok": true, "title_command_enabled": body.title_command_enabled }))
                .into_response()
        }
        Ok(_) => (
            ...
            .into_response(),
        Err(error) => {
            tracing::error!(%error, "title-command-settings POST DB-Fehler");
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "error": "db" })),
            )
...
mod tests {
    ...
    async fn partner_toggle_roundtrip_default_an() {
        ...
        let (s, j) = body_of(
            get_handler(
                partner("nani", "42"),
                State(pool.clone()),
                Query(TitleCommandQuery::default()),
            )
            .await,
        )
        .await;
        assert_eq!(s, StatusCode::OK);
        assert_eq!(j["title_command_enabled"], true);
        ...
        let (s, j) = body_of(
            post_handler(
                ...
                State(pool.clone()),
                Query(TitleCommandQuery::default()),
                Json(TitleCommandUpdate {
                    title_command_enabled: false,
                }),
            )
            .await,
        )
        .await;
        assert_eq!(s, StatusCode::OK);
        assert_eq!(j["title_command_enabled"], false);
    ...
    async fn partner_liest_nur_eigene_id_und_ignoriert_fremdes_ziel() {
        ...
        let (status, body) =
            body_of(get_handler(partner("nani", "42"), State(pool.clone()), query()).await).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(body["title_command_enabled"], false);
        post_handler(
            partner("nani", "42"),
            State(pool.clone()),
            query(),
        ...
        let foreign: i32 = sqlx::query_scalar(
            "SELECT title_command_enabled FROM streamer_plans WHERE twitch_user_id = '99'",
        )
        .fetch_one(&pool)
        .await
        .unwrap();
