...
fn quantile_exclusive(sorted: &[f64], q: f64) -> f64 {
    ...
    if x <= 0.0 { return sorted[0]; }
    let max_idx = (n - 1) as f64;
    if x >= max_idx { return sorted[n - 1]; }
...
fn robust_stats(mut per_streamer: Vec<f64>, sample_count: usize) -> SlotStats {
    ...
    if count == 0 {
        return SlotStats { median: None, p25: None, p75: None, streamer_count: 0, sample_count: 0 };
    }
    ...
    SlotStats { median: med, p25, p75, streamer_count: count, sample_count }
}
...
pub async fn category_timings_handler(
    ...
    let table = if use_tracked { "twitch_stats_tracked" } else { "twitch_stats_category" };
    ...
    let hour_rows = sqlx::query(&hour_sql).bind(since).fetch_all(&pool).await.unwrap_or_default();
    ...
    let dow_rows = sqlx::query(&dow_sql).bind(since).fetch_all(&pool).await.unwrap_or_default();
    ...
    for row in &hour_rows {
        ...
        let med: f64 = row.try_get::<Option<f64>, _>("med_vc").unwrap_or(None).unwrap_or(0.0);
        let cnt: i64 = row.try_get("cnt").unwrap_or(0);
        if !(0..24).contains(&hour) { continue; }
        hour_data.entry(hour).or_default().insert(streamer.clone(), med);
    ...
    for row in &dow_rows {
        ...
        let med: f64 = row.try_get::<Option<f64>, _>("med_vc").unwrap_or(None).unwrap_or(0.0);
        let cnt: i64 = row.try_get("cnt").unwrap_or(0);
        if !(0..7).contains(&dow) { continue; }
        dow_data.entry(dow).or_default().insert(streamer.clone(), med);
    ...
    // ── Stündliche Ausgabe ────────────────────────────────────────────────────
    let hourly: Vec<serde_json::Value> = (0..24).map(|hour| {
    ...
    let weekly: Vec<serde_json::Value> = weekday_order.iter().map(|&wd| {
