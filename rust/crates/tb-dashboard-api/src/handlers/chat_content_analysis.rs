...
pub async fn chat_content_analysis_handler(
    ...
    match tb_analytics::chat_content_analysis::load_chat_content_analysis_payload(
        &pool, &streamer, days,
    )
    .await
    {
...
mod tests {
    ...
    async fn streamer_pflicht_400() {
        let Some(pool) = make_pool("t_cca_h1").await else {
            return;
        };
        let resp = chat_content_analysis_handler(
            DashboardAuthLevel::admin(),
            State(pool),
            Query(ContentAnalysisQuery {
                streamer: None,
                days: None,
            }),
        )
    ...
    /// IDOR-Guard: Partner ohne Plan/mit fremdem ?streamer= bekommt nie fremde
    /// Daten → 403 (Plan-Gate ODER Scope-Guard greift, beide forbidden).
    #[tokio::test]
    async fn partner_fremder_streamer_403() {
        let Some(pool) = make_pool("t_cca_h3").await else {
            return;
        };
    ...
    async fn localhost_200() {
        let Some(pool) = make_pool("t_cca_h2").await else {
            return;
        };
        let resp = chat_content_analysis_handler(
            DashboardAuthLevel::admin(),
