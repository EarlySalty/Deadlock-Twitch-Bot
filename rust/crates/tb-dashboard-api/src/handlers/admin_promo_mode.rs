...
use tb_analytics::promo_mode::{
    save_global_promo_mode, validate_global_promo_mode_config, SavePromoModeError,
};
...
mod tests {
    ...
    async fn unauth_ist_auth_required_401() {
        let Some(pool) = make_pool("t_promo_unauth").await else {
            return;
        };
        let resp = set_promo_handler(
    ...
    async fn custom_event_ohne_message_400() {
        let Some(pool) = make_pool("t_promo_validation").await else {
            return;
        };
    ...
    async fn standard_speichern_200() {
        let Some(pool) = make_pool("t_promo_save").await else {
            return;
        };
        ...
        let cfg = tb_analytics::promo_mode::load_global_promo_mode(&pool)
            .await
            .unwrap();
    ...
    async fn custom_event_speichern_und_aktiv() {
        let Some(pool) = make_pool("t_promo_custom").await else {
            return;
        };
        let body = Bytes::from(
            r#"{"mode":"custom_event","is_enabled":true,"custom_message":"Event bei {invite}!"}"#,
        );
        let resp = set_promo_handler(
            DashboardAuthLevel::admin(),
            None,
