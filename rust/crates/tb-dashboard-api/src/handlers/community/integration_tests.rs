...
async fn real_database_filters_partners_and_returns_private_read_only_recommendations() {
    ...
    sqlx::raw_sql(r#"
        ...
        SELECT p.twitch_user_id, p.twitch_login, i.discord_user_id,
        ...
        INSERT INTO twitch_partners (twitch_user_id,twitch_login) VALUES ('1','Alice'),('2','Bob'),('3','archived'),('4','optout'),('5','former'),('6','paused');
        ...
        INSERT INTO twitch_stream_sessions
          ...
          FROM unnest(ARRAY['Alice','Bob','archived','optout','former','paused']) login CROSS JOIN unnest(ARRAY[2,9,16]) day;
    ...
    assert_eq!(sqlx::query_scalar::<_,i64>("SELECT COUNT(*) FROM twitch_stream_sessions").fetch_one(&db.pool).await.unwrap(),20);
    db.close().await;
}
