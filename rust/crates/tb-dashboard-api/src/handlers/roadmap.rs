...
async fn ensure_table(pool: &PgPool) -> Result<(), axum::response::Response> {
    sqlx::query(ENSURE_TABLE_SQL).execute(pool).await.map_err(|e| {
        tracing::error!("roadmap ensure-table fehlgeschlagen: {e}");
...
mod tests {
    ...
    async fn crud_pool(schema: &str) -> Option<PgPool> {
        ...
        sqlx::query(ENSURE_TABLE_SQL).execute(&pool).await.ok()?;
        Some(pool)
    }
