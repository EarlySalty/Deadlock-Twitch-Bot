...
pub fn save(
...
) -> Result<BotConfigSnapshot, EditError> {
    ...
    let current = BotConfigSnapshot::load(&source)?;
    ...
    let text = toml::to_string_pretty(&candidate).map_err(|_| EditError::Io)?;
    ...
    let result = (|| {
        ...
        if BotConfigSnapshot::load(&source)?.fingerprint() != expected_fingerprint {
            return Err(EditError::Conflict);
        }
