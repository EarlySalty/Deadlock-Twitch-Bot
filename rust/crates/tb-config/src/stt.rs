//! Betriebseigentümer für den lokalen Whisper-Dienst und seine Rust-Aufrufer.
//!
//! Der Python-Ausnahmedienst liest keine zweite Konfigurationsdatei. Der
//! Rust-Starter reicht diese vollständig geprüften Werte als Startargumente
//! weiter. Ein Modell wird geladen, aber ohne Audio kein Transkriptionsjob
//! gestartet. Änderungen an Listener, Modell und Threads verlangen Neustart.
...
pub struct SttConfig {
    ...
    /// Nur der STT-Dienst benötigt diese Startpfade. Bot und Dashboard können
    /// denselben Anschluss verwenden, ohne selbst einen Python-Prozess zu starten.
    pub launch: Option<SttLaunch>,
}

#[derive(Clone, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct SttLaunch {
    pub python_binary: PathBuf,
    pub server_script: PathBuf,
    /// Laufzeitablage der Modellgewichte, kein zweites Konfigurationsdokument.
...
impl SttConfig {
    ...
    pub fn validate(&self) -> Result<(), FileError> {
        ...
        if let Some(launch) = &self.launch {
            for (path, field) in [
                (&launch.python_binary, "stt.launch.python_binary"),
                (&launch.server_script, "stt.launch.server_script"),
                (&launch.cache_directory, "stt.launch.cache_directory"),
            ] {
