        ...
        r#"
            SELECT p.twitch_user_id,
             ...
             WHERE LOWER(TRIM(COALESCE(p.status, ''))) = 'active'
               ...
               -- Kein NULL-Fallback auf partnered_at: wer in der erfassten
               -- Historie nie einen Deadlock-Stream hatte, ist kein Pausen-,
               -- sondern ein Trenn-Fall. Die Pausen-DM verspricht "streamst du
               -- wieder Deadlock, ist der Bot zurueck", und dort gab es nie ein
               -- "wieder".
