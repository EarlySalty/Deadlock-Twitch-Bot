...
mod tests {
    ...
    async fn pdf_download_und_none_faelle() {
        ...
        sqlx::query("CREATE TABLE affiliate_gutschriften (id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, gutschrift_number TEXT, pdf_blob BYTEA)").execute(&pool).await.unwrap();
