...
impl StripeClient {
    ...
    ///
    /// `expand[]=items.data.price`, damit Lookup-Key und Price-Metadaten im
    /// Webhook-Mapping ankommen (Stripe Basil hat Perioden auf dem Item).
    pub async fn retrieve_subscription(&self, subscription_id: &str) -> Result<Value, StripeError> {
        ...
        self.get(
            &url,
            &[("expand[]".to_string(), "items.data.price".to_string())],
        )
        .await
    }
