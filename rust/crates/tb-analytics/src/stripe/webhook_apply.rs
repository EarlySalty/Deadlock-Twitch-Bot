...
pub fn streamer_plan_sync_from_event(
    ...
    if event_name == "checkout.session.completed" && str_field(event_object, "mode") == "subscription" {
        let (payload, bonus_months) =
            checkout_subscription_state_from_event("", event_object, retrieved_subscription);
        return Some(StreamerPlanSync::from_subscription_state(&payload, bonus_months));
    }
    None
}
...
mod tests {
    ...
    async fn subscription_created_sets_plan() {
        ...
        let sync = streamer_plan_sync_from_event("customer.subscription.created", &event_object, None)
    ...
    async fn plan_sync_fehler_rollt_webhook_kern_nicht_zurueck() {
        let Some(pool) = pool_or_skip("wh_sync_err_no_rollback").await else { return };
    ...
    async fn checkout_with_bonus_months_extends_manual_plan_expires_at() {
        ...
        drop(tx);
        let sync = streamer_plan_sync_from_event(
            "checkout.session.completed",
            &session,
            Some(&retrieved),
        )
