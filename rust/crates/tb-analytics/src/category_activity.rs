...
fn build_point(
    ...
    obj.insert(
        "categoryAvg".to_string(),
        json!(cat.and_then(|c| c.0).map(round1)),
    );
    obj.insert(
        "trackedAvg".to_string(),
        json!(trk.and_then(|c| c.0).map(round1)),
    );
    obj.insert("categoryPeak".to_string(), json!(cat.and_then(|c| c.1)));
    obj.insert("trackedPeak".to_string(), json!(trk.and_then(|c| c.1)));
    obj.insert(
        "categorySamples".to_string(),
        json!(cat.map(|c| c.2).unwrap_or(0)),
    );
    obj.insert(
        "trackedSamples".to_string(),
        json!(trk.map(|c| c.2).unwrap_or(0)),
    );
    Value::Object(obj)
}
...
pub async fn load_category_activity_series(pool: &PgPool, days: i64) -> Result<Value, sqlx::Error> {
    ...
    let hourly_rows: Vec<AggRow> = sqlx::query_as(&agg_sql("HOUR"))
        .bind(since)
        .fetch_all(pool)
        .await?;
    let weekday_rows: Vec<AggRow> = sqlx::query_as(&agg_sql("DOW"))
        .bind(since)
        .fetch_all(pool)
        .await?;
...
mod tests {
    ...
    async fn series_struktur_und_buckets() {
        let Some(pool) = make_pool("t_catact").await else {
            return;
        };
    ...
    async fn avg_wird_auf_1_nachkommastelle_gerundet() {
        let Some(pool) = make_pool("t_catact_round").await else {
            return;
        };
    ...
    async fn leere_tabellen_volle_struktur() {
        let Some(pool) = make_pool("t_catact_empty").await else {
            return;
        };
        let v = load_category_activity_series(&pool, 30).await.unwrap();
        assert_eq!(v["hourly"].as_array().unwrap().len(), 24);
