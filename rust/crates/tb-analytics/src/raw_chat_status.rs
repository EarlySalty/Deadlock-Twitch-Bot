...
fn iso_or_none_str(text: Option<String>) -> Value {
    text.as_deref().and_then(coerce_ts).map(emit_iso).map(Value::String).unwrap_or(Value::Null)
}
...
async fn query_scope_presence(pool: &PgPool, streamer: &str, scope: &Scope<'_>) -> Result<PresenceStats, sqlx::Error> {
    match scope {
        ...
        Scope::Sessions(ids) => {
            let (rows, sessions): (i64, i64) = sqlx::query_as(
                "SELECT COUNT(*)::bigint, COUNT(DISTINCT sc.session_id)::bigint \
                   FROM twitch_session_chatters sc \
                  WHERE LOWER(sc.streamer_login) = $1 AND sc.session_id = ANY($2)",
            )
            .bind(streamer)
            .bind(*ids)
            .fetch_one(pool)
            .await?;
            let gap: Option<DateTime<Utc>> = sqlx::query_scalar(
                "SELECT MIN(s.started_at) FROM twitch_stream_sessions s \
                  WHERE LOWER(s.streamer_login) = $1 AND s.id = ANY($2) \
            ...
            .fetch_one(pool)
            .await?;
            Ok(PresenceStats { presence_rows: rows, sessions_with_presence: sessions, gap_start: iso_or_none_dt(gap) })
        }
        Scope::Since(since) => {
            let (rows, sessions): (i64, i64) = sqlx::query_as(
                "SELECT COUNT(*)::bigint, COUNT(DISTINCT sc.session_id)::bigint \
...
async fn query_scope_raw(pool: &PgPool, streamer: &str, scope: &Scope<'_>) -> Result<RawStats, sqlx::Error> {
    let (rows, sessions, last): (i64, i64, Option<DateTime<Utc>>) = match scope {
        Scope::Sessions(ids) if ids.is_empty() => return Ok(RawStats { raw_rows: 0, sessions_with_raw: 0, last_message_at: Value::Null }),
        Scope::Sessions(ids) => sqlx::query_as(
            "SELECT COUNT(*)::bigint, COUNT(DISTINCT m.session_id)::bigint, MAX(m.message_ts) \
               FROM twitch_chat_messages m \
              WHERE LOWER(m.streamer_login) = $1 AND m.session_id = ANY($2)",
        )
        .bind(streamer)
        .bind(*ids)
        .fetch_one(pool)
        .await?,
        Scope::Since(since) => sqlx::query_as(
            "SELECT COUNT(*)::bigint, COUNT(DISTINCT m.session_id)::bigint, MAX(m.message_ts) \
               FROM twitch_chat_messages m \
    ...
    Ok(RawStats { raw_rows: rows, sessions_with_raw: sessions, last_message_at: iso_or_none_dt(last) })
}

/// Baut den `rawChatStatus`-Block (Python `build_raw_chat_status`).
pub async fn build_raw_chat_status(pool: &PgPool, streamer_login: &str, scope: Scope<'_>) -> Result<Value, sqlx::Error> {
    ...
    let health: Option<(Option<String>, Option<String>, Option<String>, Option<String>)> = sqlx::query_as(
    ...
    let fallback_last: Value = match sqlx::query_scalar::<_, Option<DateTime<Utc>>>(
        "SELECT MAX(message_ts) FROM twitch_chat_messages WHERE LOWER(streamer_login) = $1",
    )
    .bind(&streamer)
    .fetch_one(pool)
    .await
    ...
    let health_last_message_at = iso_or_none_str(health.as_ref().and_then(|h| h.0.clone()));
    let health_last_insert_ok_at = iso_or_none_str(health.as_ref().and_then(|h| h.1.clone()));
    let health_last_insert_error_at = iso_or_none_str(health.as_ref().and_then(|h| h.2.clone()));
    let health_last_error: Option<String> = health
        .as_ref()
        .and_then(|h| h.3.clone())
    ...
    let backfill_row: Option<(Option<String>, Option<String>)> = sqlx::query_as(
    ...
    let backfill_state = if let Some((status, _note)) = &backfill_row {
        let s = status.as_deref().unwrap_or("").trim();
        if s.is_empty() { "not_started".to_string() } else { s.to_string() }
    ...
    let last_message_at = first_non_null([raw.last_message_at.clone(), health_last_message_at, fallback_last]);
...
mod tests {
    ...
    async fn leerer_streamer_default() {
        let Some(pool) = make_pool("t_rcs_empty", true).await else { return };
        let v = build_raw_chat_status(&pool, "  ", Scope::Since(Utc::now())).await.unwrap();
    ...
    async fn presence_ohne_raw_meldet_luecke() {
        let Some(pool) = make_pool("t_rcs_gap", true).await else { return };
        ...
        let v = build_raw_chat_status(&pool, "nani", Scope::Since(since)).await.unwrap();
        assert_eq!(v["available"], false);
    ...
    async fn raw_vorhanden_available() {
        let Some(pool) = make_pool("t_rcs_ok", true).await else { return };
        sqlx::query("INSERT INTO twitch_stream_sessions (id, streamer_login, started_at) VALUES (1,'nani',NOW()-INTERVAL '1 day')").execute(&pool).await.unwrap();
        sqlx::query("INSERT INTO twitch_session_chatters (session_id, streamer_login, chatter_login) VALUES (1,'nani','viewer')").execute(&pool).await.unwrap();
    ...
    async fn diag_tabellen_fehlen_graceful() {
        ...
        let Some(pool) = make_pool("t_rcs_nodiag", false).await else { return };
        sqlx::query("INSERT INTO twitch_stream_sessions (id, streamer_login, started_at) VALUES (1,'nani',NOW()-INTERVAL '1 day')").execute(&pool).await.unwrap();
        ...
        let v = build_raw_chat_status(&pool, "nani", Scope::Since(Utc::now() - chrono::Duration::days(30))).await.unwrap();
        assert_eq!(v["available"], true);
