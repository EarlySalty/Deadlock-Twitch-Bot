...
STATUS sqlx-Offline 2026-09-05: `cargo sqlx prepare --workspace` gegen die migrierte Test-DB (Port 33097) gelaufen. Sechs neue Cache-Dateien für die neuen `query!`/`query_scalar!` (record_pitch_log-INSERT, load_recent_channel_messages, load_live_context, load_stream_start, pitch_user_limit_ok, pitch_channel_limit_ok). Der Lauf löschte zusätzlich fünf unbeteiligte Cache-Einträge (feature-gated DDL in tb-analytics/tb-engagement, in diesem Prepare-Lauf nicht kompiliert); diese fünf wurden per `git checkout` zurückgeholt, weil die zugehörigen Queries im Code noch existieren und ein fehlender Cache-Eintrag den Offline-Build dieser Pfade brechen würde. Verifiziert: `SQLX_OFFLINE=true cargo build -p tb-chat` (ohne DATABASE_URL) EXIT=0. HINWEIS Scope: `rust/.sqlx/` steht nicht im "Erlaubten Änderungsbereich" des Contracts; die sechs neuen Cache-Dateien sind mechanische Folge der neuen Queries und müssen mit rein, sonst bricht der Offline-Release-Build. Vor dem Merge-Gate braucht es dafür (und für den Schema-Snapshot, siehe Deploy) eine vom Nutzer angelegte Freigabedatei `~/.claude/.contract-approvals/2026-09-04-discord-pitch-qualitaet` mit den Pfaden `rust/.sqlx` und `rust/crates/tb-db/tests/fresh_schema_snapshot.txt`.
...
STATUS M5 erledigt 2026-09-05: `on_message_pitch` in promos.rs voll implementiert (Vorfilter 25 Zeichen/kein Befehl/kein Broadcaster, Gates Partner/Allowlist/Werbefrei/Suppression/Startverzögerung, Limits User 7 Tage und Kanal 3 pro Stream mit 10 min Abstand über `twitch_promo_pitch_log`, Doppelsend-Lock, Judge-Seam, harte Filter, `@login`-Send, Cooldown-Kopplung `mark_promo_sent("anlass_pitch")`, Review-Karte). Helfer `log_anlass_reject`, `pitch_user_limit_ok`, `pitch_channel_limit_ok`, `load_stream_start`. pipeline.rs: detached `tokio::spawn(on_message_pitch)` im `is_deadlock_live`-Block direkt nach `promos.on_message` (blockiert die Pipeline nicht). Logging-Politik: Gate-Blocks (Partner, Allowlist, Werbefrei, Suppression, Startverzögerung), die Pre-Judge-Limitprüfungen (User 7 Tage, Kanal 3 pro Stream) und kein_anlass werden nicht geloggt, nur `tracing::debug`, damit die Log-Tabelle nicht flutet. Eine Zeile in `twitch_promo_pitch_log` entsteht nur, wenn das Modell einen Text erzeugt hat: harte Filter-Verwerfungen, die Post-Judge-Kanal-Limit-Verwerfung unter dem Lock, Send-Drops und gesendete Pitches. M7-Regressionstests nach dem Rot-Lauf grün: `anlass_pitch_symphooniee_wird_gesendet`, `anlass_pitch_bei_promo_disabled_sendet_nichts`, `anlass_pitch_kurznachricht_ohne_llm`, `anlass_pitch_harter_filter_verwirft_link` = 5 passed (inkl. Parser). Guard-Test grün.
...
3. `cargo sqlx prepare` muss vor dem Release-Build gelaufen sein (M5), sonst bricht der Offline-Build. Ist im Branch erledigt (sechs neue `.sqlx`-Dateien). Zusätzlich muss der Schema-Snapshot `rust/crates/tb-db/tests/fresh_schema_snapshot.txt` die neue Tabelle enthalten, sonst schlägt `tb-db --test fresh_migrations_schema` (und damit `sqlx-prepare.sh --check`) fehl: `UPDATE_SCHEMA_SNAPSHOT=1 TEST_DATABASE_URL=<frisch migrierte DB> cargo test -p tb-db --test fresh_migrations_schema` laufen und die eine neue Zeile (twitch_promo_pitch_log) committen. Snapshot und `.sqlx` liegen außerhalb des Contract-Scopes; die Freigabedatei aus dem M3-Hinweis muss vor dem Gate stehen.
