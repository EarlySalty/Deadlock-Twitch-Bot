...
- Befund 1 (BLOCKING, Raid-Erfolg): behoben in 448e7007 (`karte_raids` partitioniert nach `success` (null/fehlend gilt wie im Datenproduzenten `raid_events_block` als erfolgreich), Kopfzeile "Raids ausgeführt: N, nicht zustande gekommen: M", Liste nur erfolgreiche Raids; Test `karte_raids_zaehlt_erfolg_getrennt` mit Fixtures true/false/null).
- Befund 2 (NIT, Bot-Status): behoben in 448e7007 (`karte_partner` leitet "Bot aktiv" aus `partner_status == "active"` UND `operational_state == "active"` ab, gibt bei gesetztem `technical_pause_reason` eine Pausengrund-Zeile in Nutzersprache aus (`pausengrund_text`: blocked/token_error/bot_banned plus generisch); Test `karte_partner_erkennt_pause_trotz_aktivem_status`).
- Befund 3 (NIT, geloggte Frage): behoben in 448e7007 (Log-Aufruf schreibt `q_clean` statt `question`, also die tatsächlich ans Modell gestellte Frage).
- Befund 4 (NIT, Log auf Fehlerpfaden): behoben in 448e7007 (Helfer `spawne_log`; Log-Eintrag auf 429 mit `answer = "[abgelehnt: rate_limited]"` und auf 502 (leerer Text und Err) mit `answer = "[fehler: model_unavailable]"`, provider/model leer; 400 bleibt ungeloggt).
- Befund 5 (NIT, Ban-Zähler): behoben in 448e7007 (`ban_count_block` aus `ban_events_block` in internal_home.rs herausgezogen, ohne dessen Verhalten zu ändern; `ban_events_block` nutzt die Zählfunktion intern; die Karte ruft `ban_count_block`, das teure `LIMIT 20`-Listen-Query entfällt für die Karte).
- Befund 6 (NIT, Seiten-Slug): behoben in 448e7007 (`slug` als `[slug, setSlug]`, `setSlug(seiteSlug())` beim Öffnen des Panels, `seiteSlug()` erneut beim Senden für `page`; Vorschläge folgen dem aktuellen Slug; Quelltext-Vertrag in tests ergänzt).
- Befund 7 (NIT, Login-Gate): behoben in 448e7007 (`DashboardAssistent` liest `isLoading` mit und rendert `null`, solange der Auth-Status lädt oder `authStatus?.authenticated !== true`; nutzt denselben `useAuthStatus`-Query-Key, kein zweiter Fetch; Quelltext-Vertrag in tests ergänzt).
- Befund 8 (NIT, Quelltext-Test): behoben in 448e7007 (Antwort-Aufbau in reine Funktion `baue_antwort(answer, parts, sources, grounded, page) -> Value` gezogen; `antwort_meldet_echten_grounded_wert` testet jetzt die Funktion statt `include_str!` auf den eigenen Quelltext).

Validierung Fix-Runde 3: `cargo test -p tb-dashboard-api -p tb-analytics` EXIT 0 (459 + 1114 (1 ignored) + 12 passed, 0 failed, plus Hilfs-Suiten); `cargo clippy -p tb-dashboard-api -- -D warnings` EXIT 0, sauber; `npm test` 193 passed 0 fail; `npm run lint` 0 errors (16 vorbestehende Warnungen in nicht angefassten Dateien); `npm run build` grün.
