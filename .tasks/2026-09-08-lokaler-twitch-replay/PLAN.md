...
Der erste Testlauf vor Implementierung war rot: `cargo test -p tb-llm --features local-eval --test local_eval`, fehlender Bin und `unresolved import tb_llm::local_eval`. Danach Implementierung begonnen. Unabhängiger Rust-/Securityreview folgt vor echten Läufen.
...
- Je Modell kurzer Sanity-Lauf, danach 50 gleiche Fälle und zehn Baselines, sofern gemessene Laufzeit dies zulässt. Ein Modell zur Zeit; Produktionslast beobachten.
