...
- 2026-09-05 M3 grün: Umbenennung (index.html Titel, Header-Badge, Home-Button, Home-Hinweistext, PricingTour-Knopf, Backlink-Woerterbuch) plus `tests/partnerDashboardName.test.ts`. `npm test` -> tests 228, pass 228, fail 0. Rot-Gegenprobe: Header-Badge auf "Twitch Analytics" sabotiert -> partnerDashboardName 2 fail statt 0; nach Revert 7 pass.
- 2026-09-05 M4 grün: `npm run build:preview` -> built ok, `dist-preview/index.html` vorhanden; Titel im Artefakt "Partner Dashboard · Deutsche Deadlock Community".
