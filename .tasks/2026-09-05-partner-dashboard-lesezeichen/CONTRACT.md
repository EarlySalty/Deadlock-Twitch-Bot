...
- 2026-09-05, REQ4 und REQ5, alt: REQ4 verlangt Tastenkombination und "Link kopieren" unabhängig vom Browser, REQ5 nimmt mobil nur die Adressleisten-Nachbildung weg. Neu: Mobil (REQ5) entfällt zusätzlich die Tastenkombination, weil es auf Telefonen keine Tastatur gibt; der Knopf "Link kopieren" bleibt auch mobil sichtbar. Grund: Gate-Fund zu LesezeichenHinweis.tsx:220, technische Klarstellung ohne Produktänderung, entschieden von Orchestrator (technisch, reversibel).
- 2026-09-05, REQ2, Ergänzung: Die reine Erkennungsfunktion wertet das Eingabefeld `mobile` (aus `navigator.userAgentData.mobile`) zusätzlich zur User-Agent-Zeichenkette aus; ist eines von beiden mobil, gilt mobil. Grund: Gate-Fund zu browserErkennung.ts:77 (Signal war tot), entschieden von Orchestrator (technisch, reversibel).
