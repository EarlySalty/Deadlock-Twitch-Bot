...
5. Doppelter Kontakt Partner-Pitch gegen Outreach-Kette: der eigene Ledger-Insert (action='posted') plus das Lifetime-Limit (pfad='partner') sperren jede zweite Ansprache derselben user_id durch UNSEREN Partner-Zweig (partner_candidate liest den Ledger). KORREKTUR (Review): tb-scout liest twitch_scout_pitch_ledger nicht (Detector detector.rs:53-101 prüft twitch_partners, twitch_raid_blacklist, twitch_partner_signup_denylist, twitch_scout_pitch_blacklist, twitch_outbound_chat_suppressions, twitch_partner_outreach, twitch_scout_candidates, nie den Ledger; einziger Ledger-Leser im Baum ist promos.rs:partner_candidate). REQ-06 verlangt nur den Ledger-Eintrag, den wir schreiben; dass die Scout-Kette ihn nicht auswertet, liegt in tb-scout und ist durch INV-04 gesperrt, also nicht Teil dieser Aufgabe.
...
- M5 Review-Fixes: BLOCKING-Klasse (pfadübergreifende Limits) behoben. pitch_user_limit_ok und pitch_channel_limit_ok zählen jetzt pfad IN ('anlass','partner') (7-Tage-Sperre je Zuschauer und 600s/3-Slot je Kanal gelten pfadübergreifend; INV-03 bleibt, weil ohne Partner-Zeilen identisch). partner_channel_limit_ok blockt zusätzlich, wenn irgendein Pitch (Anlass oder Partner) im Kanal jünger als 600s ist. Lifetime-Limit zusätzlich unter dem Sendelock erneut geprüft (NIT 6). Neuer Test partner_pitch_schreibt_ledger_und_review_karte deckt Ledger-Zeile und Partner-Review-Karte ab (NIT 7). PLAN-Risiko 5 als Review-Korrektur richtiggestellt (NIT 5). NIT 4 (Kandidat mit erschöpften Partner-Limits bekommt auch keinen Anlass-Pitch) bleibt bewusst per Plan-Entscheidung D4.
