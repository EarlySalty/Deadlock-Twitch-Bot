...
Ersetzt `.tasks/2026-09-05-analyse-backend-bots/` (überholt: Backfill ohne Zeitfenster, Chatter-Bedingung in REQ-05) und `.tasks/2026-09-05-analyse-bots-sprache-chatluecke/` (überholt: 4 statt 20 Bot-Listen).
...
- REQ-04: Neue Sessions bekommen ihre Sprache beim Anlegen aus den Helix-Stream-Daten (Ursache der 1029 leeren `language`-Werte ist im Session-Schreiber belegt und behoben). Bestehende Sessions mit leerer Sprache werden per einmaligem SQL-Update nachgefüllt: je Session der jüngste Wert aus `twitch_channel_updates` derselben `twitch_user_id` mit `language <> ''` und `recorded_at <= ended_at`; Sessions ohne passenden Wert bleiben leer. Das Update liegt als Datei `backfill.sql` im Task-Ordner, wird vorher als Zähl-SELECT belegt und vom Orchestrator als `postgres` ausgeführt.
- REQ-05: Die Chat-Lücken-Warnung (`suspectedIngestionIssue`) zählt nur Sessions als Lücke, deren Dauer mindestens 10 Minuten beträgt. Geister-Sessions aus EventSub-Flaps (Dauer unter 10 Minuten) lösen sie nicht mehr aus. Beleg: Regressionstest, der mit zwei Sessions unter 10 Minuten ohne Chat plus einer echten Session mit Chat vor dem Fix `true` liefert und danach `false`; roter Lauf mit Testname und Fehlermeldung in `PLAN.md` festgehalten.
...
- INV-02: Kein Schema-Wechsel, keine Migration; REQ-04 ist reines Daten-Update, das nur Zeilen mit leerer oder NULL-Sprache anfasst.
...
- rust/Cargo.lock
- .tasks/2026-09-05-backend-bots-sprache/
...
- 2026-09-05: Praezisierung des vorigen Eintrags alt (nur geaenderte Abfragen) -> neu (zusaetzlich fuenf tote Orphan-Cache-Dateien entfernt, die kein lebendes query!-Makro mehr tragen: 0df9a0dd, 13578ee1, 762f88ce, f0749920, fad7e773), Grund: cargo sqlx prepare raeumt verwaiste Eintraege mit; Offline-Build von tb-bot und tb-dashboard bleibt gruen, entschieden von Orchestrator
- 2026-09-05: Erlaubter Bereich alt (ohne die drei Dashboard-chat-Handler) -> neu (zusaetzlich rust/crates/tb-dashboard-api/src/handlers/chat_social_graph.rs, chat_content_analysis.rs, chat_hype_timeline.rs), Grund: nur die Test-Fixtures (make_pool) bekommen prod-treue Spalten ended_at/duration_seconds, weil sie build_raw_chat_status mit der neuen Gap-Query aufrufen; ohne die Spalten bricht ein Test, sobald er eine Session mit Chat einfuegt (heute nicht getroffen). Produktivcode dieser Dateien bleibt unberuehrt, entschieden von Orchestrator
