...
UPDATE twitch_stream_sessions s SET stream_title = w.titel FROM wahl w WHERE w.id = s.id AND w.titel IS NOT NULL AND (s.stream_title IS NULL OR s.stream_title = '');
