...
Die zusätzliche lokale Uplink-Browserprobe scheiterte zuerst nach 45,3 Sekunden mit `CDP-Frist: Page.navigate`. Nach Abschluss der lokalen Rust-Testkompilierung bestand der unveränderte Nachweis in 14,3 Sekunden mit `assertions: passed`. Es wurden weder Fristen erhöht noch Assertions abgeschwächt. Die erste Zeitüberschreitung bleibt als nicht abschließend erklärte Instabilität dokumentiert; der Wiederholungserfolg ist kein Beleg, dass ihre Ursache behoben wurde. Logs: `/tmp/tb953-uplink-browser-20260924.log` und `/tmp/tb953-uplink-browser-recheck-20260924.log`.

Der Descriptor-basierte Start prüft und startet dasselbe geöffnete Chromium-Artefakt. Der erfolgreiche zweite Lauf umfasst die vorhandenen Uplink-Bedienassertionen mit synthetischen Daten und ausschließlich lokalem Testserver. Der neue CodeQL-Befundstatus muss weiterhin auf GitHub geprüft werden.
...
18 Gate-Unit-Tests, vier Schema-Schutztests und elf SARIF-Policy-Tests wurden mit `rustc +1.98.0 --edition=2021 -D warnings --test` neu kompiliert und bestanden. Der echte Gate-Prozess bestand zwei positive und 78 negative Gegenproben. Fehlende Prüfungen, Fehler, Abbruch und unerwartete Skips werden nicht als Erfolg umgedeutet.

Zusätzlich bestanden 46 gezielte Rust-Laufzeittests unter `cargo +1.98.0 test --locked -j2`: zehn Zuschauerregister-, acht Community-Matching-, 18 Self-Explainer- und zehn Outreach-Shadow-Tests. Je Lauf gab es null Fehler und null ignorierte Tests; die übrigen Bibliothekstests waren durch die benannten Filter nicht ausgewählt. Belege: `/tmp/tb953-rust-targeted-20260924/results.json` und die vier Logs. Das ist noch kein vollständiger aktueller Workspace-/DB-/GitHub-Nachweis.

`cargo +1.98.0 fmt --all --check` wurde ebenfalls erneut ausgeführt: Exit 1, 258 unterschiedliche Dateien. Der rohe Nachweis steht in `/tmp/tb953-format-20260924.log`, die Zählung in `.json`. Es wurde kein großflächiger Formatierungsdiff erzeugt und keine Gate-Ausnahme ergänzt. `actionlint 1.7.12` prüfte die aktuellen Workflow-Dateien erfolgreich mit Exit 0.
...
Die drei bisherigen Basismigrations-Hashes und beide Brain-Migrations-Hashes stimmen im öffentlichen Manifest mit den bereits im Twitch-Verbraucher fixierten Werten überein. Zusätzlich wurden die drei Basisdateien anonym heruntergeladen und ihre tatsächlichen Bytes mit SHA-256 gegen die bestehenden Werte geprüft; drei von drei stimmen überein. Beleg: `/tmp/tb953-public-brain-schema-20260924/verification.json`. Die Rückmeldung steht in https://github.com/EarlySalty/Deadlock-Brain/pull/10#issuecomment-5806027554. Der vollständige neue Einstieg enthält zusätzlich drei kanonische Brain-Migrationen, läuft atomar und verlangt eine leere Datenbank mit eigenem `ci`- oder `test`-Namenssegment. Der bestehende Twitch-Verbraucher akzeptiert dagegen `twitchbot_sqlx` oder `sqlx_prepare` und lädt seine Basismigrationen noch aus dem privaten Ursprung. Diese Schnittstelle muss gezielt integriert werden; der öffentlich dokumentierte Export allein macht den unveränderten GitHub-Job nicht grün. Der Produkt-Dependency-Pin wurde nicht geändert, das Brain-Repository nicht beschrieben.
