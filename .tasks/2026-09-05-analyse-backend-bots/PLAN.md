...
Änderungen: Test in `rust/crates/tb-analytics` gegen `build_raw_chat_status` mit Fixture: zwei Sessions unter 10 Minuten, eine echte Session mit Chat (Amendment REQ-05: keine Chatter-Bedingung).
...
### M4: Sessionsprache
Änderungen: Session-Schreiber setzt `language`; `audience_demographics.rs` filtert leere Sprache; `backfill.sql` im Task-Ordner (Amendment REQ-04: `language <> ''` und `recorded_at <= ended_at` je Session).
