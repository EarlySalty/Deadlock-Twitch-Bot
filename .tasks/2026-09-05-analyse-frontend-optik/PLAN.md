...
- Merge-Gate-Fix-Runde (HEAD cbb6b4b4, rebased auf origin/main a5b2c1a2): (1) BLOCKING `chatAnalyticsShared.tsx:25` `note` aus dem Sichtbarkeits-Gate entfernt; geteilte Text-Konstanten CHAT_LUECKE_TITEL/TEXT und CHAT_KEINE_TITEL/TEXT dort exportiert und in `Viewers.tsx` importiert; `RawChatGapNotice` und der Aufrufer gaten nur noch über `suspectedIngestionIssue`/`available===false` (lokales `chatLueckeSichtbar` in Viewers, `note`-Prop entfernt). Das Prädikat blieb in Viewers lokal, weil ein Funktions-Export aus der Komponenten-Datei `react-refresh/only-export-components` bricht (Konstanten sind via vite-Config erlaubt). (2) Palette in `AudienceDemographics.tsx:51`, `ViewerProfiles.tsx:10`, `Viewers.tsx:49-53` umsortiert auf primary/success/warning/secondary/accent, sodass keine zwei Goldtöne nebeneinander liegen (Donut, Legende, Segmentkarten identisch). (3) `Header.tsx` Blur übernimmt nur bei Abweichung von `days` (keine Doppel-Refetch-Welle vor Preset-Klick). (4) `CalendarHeatmap.tsx` Fußzeile "{n} Streams in den letzten {days} Tagen". (5) `Rise.tsx` ruft ein durchgereichtes `onAnimationEnd` mit auf. Build/Lint(0 Errors)/252 Tests grün. Beobachtung Headless-Preview: der Audience-Ueberblick zeigt aktuell kein "Audience Demographics", weil `demographicsData` im Demo-Snapshot leer ist (`Audience.tsx:179`, `demographicsData &&`); Audience.tsx und der Daten-Hook sind von diesem Branch unberührt, WatchTime rendert, keine JS-Fehler. Reine Demo-Daten-Frage, keine Regression dieser Änderungen.
- Fix-Runde nach Review/Sichtprüfung (HEAD 84787c0b): (1) BLOCKING App.tsx Demo-Gate: der abgelehnte URL-Streamer blockierte den Auth-Fallback dauerhaft; `pendingUrlStreamer` wird jetzt in beiden Zweigen genullt, Freigabe-Prüfung als reine Funktion `streamerAusUrlErlaubt` in `src/utils/zeitraum.ts` ausgelagert und in `tests/zeitraum.test.ts` getestet (App.tsx ist in node nicht importierbar). (2) `src/pages/Viewers.tsx:49-53` SEGMENT_CONFIG-Balken und -Badges auf Tokens (primary/accent/warning/success/secondary) statt Cyan/Rot/Neongrün. (3) Jargon raus: `chatAnalyticsContent.tsx:94` Banner → "Zahlen nur eingeschränkt belastbar" + Nutzertext; `AudienceDemographics.tsx:78,118` Badge → "Datenbasis: echte Chat-Daten · Verlässlichkeit: hoch" (method-Ausprägungen "keine/wenige/echte Chat-Daten"). (4) Donuts `ViewerProfiles.tsx` und `AudienceDemographics.tsx` auf w-48 (192px), outerRadius 90 %, innerRadius 58 % (Durchmesser > 160px, REQ-08). Build/Lint(0 Errors)/246 Tests grün, Headless-Beleg 365+Audience+Datenbasis erneut bestätigt.
