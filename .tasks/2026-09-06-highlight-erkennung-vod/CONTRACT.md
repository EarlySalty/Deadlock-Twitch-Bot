...
- 2026-09-06: Erlaubter Änderungsbereich Erweiterung: zusätzlich `rust/crates/tb-social-media/src/bin/` und `rust/crates/tb-social-media/Cargo.toml`. Grund: die Korpus-Ernte (REQ-09, Deadlock-Clips über Helix) wird als eigenständiges Bin `korpus_ernte` verdrahtet (App-Token aus Infisical, JSON auf stdout), statt in twitch_clips_social_media zu schreiben; `register_corpus_clip` und die `corpus`-CHECK-Erweiterung entfallen dadurch. `highlight-detector lernen --korpus-datei <json>` nimmt Partner-Clips aus der DB plus diese Datei. Entschieden vom Orchestrator/Koordinator.
