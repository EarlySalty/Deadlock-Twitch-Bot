...
- Erwartet: Test rot. Der Test ruft `calculate_health_scores` mit der neuen Semantik auf (Bindung als Prozentwert 58,5 statt 10m-Retention, 25/15 Raids bei 44 Sessions) und erwartet Retention 58, Network 45. Gegen den alten Formelstand ist der zweite Parameter noch die 10m-Retention, also `retention = min(100, 58,5 * 1,5) = 87`; die Signatur bleibt `f64`, deshalb kompiliert der Test schon vor dem Fix und läuft echt rot statt als Compile-Fehler.
- Roter Lauf: `test handlers::overview::tests::retention_score_saettigt_nicht_bei_88_prozent ... FAILED`, panic bei `overview.rs:978`: `left: 87 / right: 58` (Retention-Assertion; Network-Assertion wird nach der ersten Panic nicht mehr erreicht).
- Baseline (unveränderter Produktivcode): tb-analytics 477 passed / 0 failed, tb-dashboard-api 1115 passed / 0 failed (vor M1); keine vorbestehenden roten Tests.
