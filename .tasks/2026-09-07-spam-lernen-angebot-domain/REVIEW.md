...
Nit 6 mit erledigt: die verbliebenen Kommentare "Schritt 1" bis "Schritt 4" in `calculate_spam_score` entfernt (die Kommentare "Schritt 5" und "Schritt 6+7" fehlten bereits), reiner Kommentar-Diff, Repo-Regel keine Code-Kommentare. Dabei fielen die Verweise auf die Portquellen (moderation.py Z. 494-499 usw.) weg; der Funktions-Doccomment nennt weiterhin die Gesamtspanne.

Tradeoff durch den Regex: `\s+\.\s*` weitet die Domain-Fläche gegenüber `.replace(" .", ".")` leicht, weil jetzt auch die Form mit Leerzeichen nach dem Punkt (bei Leerzeichen davor) zusammenzieht. Ein Satz wie "more subs for you . me and friends" wird dadurch zu "more subs for you.me and friends" und über die kurze TLD "me" lernbar. Der Radius bleibt eng: Lernen setzt weiter ein Richter-Fehlurteil voraus, und die gespeicherte Ganzphrase trifft danach nur genau diesen Satz (Kompaktform), nie ein Fragment. Kein Handlungsbedarf für den Merge, steht hier neben der INV-02-Grenze.
