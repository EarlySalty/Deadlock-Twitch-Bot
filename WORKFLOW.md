...
- 2026-05-25: Aufgabe "Step 8 (reduziert): Legacy Cleanup-Rest" aufgenommen. Verbleibende Legacy-Admin-/Stats-Verweise in `bot/dashboard/pages.py`, `bot/admin_dashboard/src/components/layout/Sidebar.tsx` und `bot/dashboard/server_v2.py` per `rg` geprüft; `_resolve_legacy_stats_url` hat noch einen Test-Caller und bleibt daher unberührt.
- 2026-05-25: `Stats (Alt)`-Card aus `build_stats_entry_page` entfernt und Legacy-Link-Card am Ende der React-`Sidebar` gestrichen. Verifikation per `py_compile`, `npx tsc --noEmit` und `npm run build` als nächster Schritt.
- 2026-05-25: Verifikation fuer Step 8 gruen: `python3 -m py_compile bot/dashboard/routes_entry.py bot/dashboard/pages.py bot/dashboard/server_v2.py bot/dashboard/routes_mixin.py`, `cd bot/admin_dashboard && npx tsc --noEmit`, `npm run build`. Hinweis: `tests/test_dashboard_page_templates.py` referenziert weiterhin das bereits zuvor entfernte `legacy_url`-Argument; Tests blieben gemaess Scope unberuehrt.

- 2026-05-25: Aufgabe "Step 6: Money + Audit-Log" aufgenommen. Audit-Log-Plan, `bot/analytics/api_admin.py`, Storage-/Billing-/Streamer-Admin-Quellen und Money-Frontend-Pages geprueft; reale Audit-Quellen auf `twitch_partners`, `streamer_plans`, `twitch_billing_events`, `twitch_global_promo_modes`, `roadmap_body.json` und `legal_pages.json` eingegrenzt.
- 2026-05-25: Step 6 umgesetzt: neuer Backend-Aggregator `bot/analytics/audit_log.py` plus Route `GET /twitch/api/admin/audit-log`; Frontend um `src/pages/money/AuditLog.tsx`, Client-/Hook-/Typen und Route erweitert; bestehende Money-Pages auf gemeinsames `PageHeader`-Pattern gezogen.
- 2026-05-25: Verifikation fuer Step 6 gruen: `python3 -m py_compile bot/analytics/api_admin.py bot/analytics/audit_log.py`, `cd bot/admin_dashboard && npm install`, `npx tsc --noEmit`, `npm run build`.
